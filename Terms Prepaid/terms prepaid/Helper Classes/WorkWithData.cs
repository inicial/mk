...
order by OP_ID desc"; // @dlKey int
      ...
      ,[actions_id]
      ,[Text]
      ,CASE WHEN actions_id = -112 THEN 'ID тарифа в кр. ком.' ELSE CDP_NAME END AS CDP_NAME
      ,CDP_ORDER
      ,CDP_BLOCK
  FROM [dbo].[mk_actions_options] 
  inner join dbo.MK_Cruise_DOPServise on actions_id=CDS_ID
...
(SELECT --row_number() over (order by mp_problemCode) as rownum
        ...
        public static int? GetRegionByDogovor(string dgcode)
       {
        ...
        public static DataTable GetDaysTasks()
        {
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter("mk_GetDayTasks",Connection))
            {
                ...
                {
                    TpLogger.Debug("GetDaysTasks - adapter.SelectCommand", ex);
                }
            }
            return dt;
        }
  ...
  FROM [dbo].[AnnulReasons]", Connection))
        ...
        public static bool CountNewMessage(string dg_code, ref int manager, ref int client)
        {
            DataTable dt = new DataTable();
            using (
                SqlDataAdapter adapter =
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    string mode = row.Field<string>("HI_MOD").ToUpper();
                    if (mode == "MTM") manager++;
                    if (mode == "WWW") client++;
                }
                return true;
        ...
        public DataTable GetCanceledVisas(string dgcode)
        {
            DataTable dt = new DataTable();
            string query = "select * from History where hi_dgcod =@dgcode AND HI_TEXT LIKE 'Визы:%'";
            using (SqlDataAdapter adapter = new SqlDataAdapter(query, Connection))
            { 
                adapter.SelectCommand.Parameters.AddWithValue("@dgcode", dgcode);
                adapter.Fill(dt);
...
_connectionTS = LantaSQLConnection.Open_LantaSQLConnection("total_services", user, pass, out stateTS);
            ...
            //int timeout_1 = _connection.ConnectionTimeout;
            //int timeout_2 = _connectionTS.ConnectionTimeout;
        ...
        public DateTime GetBronirDate(string dgCode)
        {
            // Дата бронирования путевки
            DateTime BronDT = new DateTime(2000, 1, 1); // дата бронирования

            string qry_dt = "SELECT [OP_LastUpdate], CONVERT(int,[OP_IsBook]) AS OP_IsBook ";
            qry_dt = qry_dt + " FROM [dbo].[mk_options] ";
            qry_dt = qry_dt + " WHERE OP_DLKEY IN ";
            qry_dt = qry_dt + "   (select dl_key from tbl_DogovorList where dl_dgcod = @dgcode) ";
            qry_dt = qry_dt + " ORDER BY [OP_LastUpdate] DESC";

            using (SqlDataAdapter adapter = new SqlDataAdapter(qry_dt, Connection))
            {
                adapter.SelectCommand.Parameters.AddWithValue("@dgcode", dgCode);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            DateTime row_dt = row.Field<DateTime>("OP_LastUpdate");
                            int row_flag = row.Field<int>("OP_IsBook");
                            if (!(row_flag > 0)) break;
                            if (row_dt < BronDT || BronDT.Year <= 2000) BronDT = row_dt;
                        }
                    }
                }
            }
            if (BronDT.Year > 2000) // если есть дата бронирования
                return BronDT; // передать значение в данные для бронирования

            return new DateTime(2000, 1, 1);
        }

        public DateTime GetDocumentQueryDate(string dgCode)
        {
            // Дата запроса документов
            DateTime DocQueryDT = new DateTime(2000, 1, 1); // дата запроса

            string qry_dt = "SELECT [OP_LastUpdate], CONVERT(int,[OP_DOCUMENT_QUERY]) AS OP_DOCUMENT_QUERY ";
            qry_dt = qry_dt + " FROM [dbo].[mk_options] ";
            qry_dt = qry_dt + " WHERE OP_DLKEY IN ";
            qry_dt = qry_dt + "   (select dl_key from tbl_DogovorList where dl_dgcod = @dgcode) ";
            qry_dt = qry_dt + " ORDER BY [OP_LastUpdate] DESC";
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter(qry_dt, Connection))
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            DateTime row_dt = row.Field<DateTime>("OP_LastUpdate");
                            int row_flag = row.Field<int>("OP_DOCUMENT_QUERY");
                            if (!(row_flag > 0)) break;
                            if (row_dt < DocQueryDT || DocQueryDT.Year <= 2000) DocQueryDT = row_dt;
                        }
                    }
                }
            }
            if (DocQueryDT.Year > 2000) // если есть дата запроса
                return DocQueryDT; // передать значение в данные для путевки

            return new DateTime(2000, 1, 1);
        }

        public DateTime GetDocumentGetDate(string dgCode)
        {
            // Дата получения документов
            DateTime DocGetDT = new DateTime(2000, 1, 1); // дата получения

            string qry_dt = "SELECT [OP_LastUpdate], CONVERT(int,[OP_DOCUMENT_GET]) AS OP_DOCUMENT_GET ";
            qry_dt = qry_dt + " FROM [dbo].[mk_options] ";
            qry_dt = qry_dt + " WHERE OP_DLKEY IN ";
            qry_dt = qry_dt + "   (select dl_key from tbl_DogovorList where dl_dgcod = @dgcode) ";
            qry_dt = qry_dt + " ORDER BY [OP_LastUpdate] DESC";
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter(qry_dt, Connection))
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            DateTime row_dt = row.Field<DateTime>("OP_LastUpdate");
                            int row_flag = row.Field<int>("OP_DOCUMENT_GET");
                            if (!(row_flag > 0)) break;
                            if (row_dt < DocGetDT || DocGetDT.Year <= 2000) DocGetDT = row_dt;
                        }
                    }
                }
            }
            if (DocGetDT.Year > 2000) // если есть дата запроса
                return DocGetDT; // передать значение в данные для путевки

            return new DateTime(2000, 1, 1);
        }

        public bool GetBronirDocumentDates(string dgCode, ref DateTime BronirDT, ref DateTime DocQueryDT, ref DateTime DocGetDT)
        {
            // Дата бронирования путевки
            BronirDT = new DateTime(2000, 1, 1); // дата бронирования
            // Дата запроса документов
            DocQueryDT = new DateTime(2000, 1, 1); // дата запроса
            // Дата получения документов
            DocGetDT = new DateTime(2000, 1, 1); // дата получения

            string qry_dt = "SELECT [OP_LastUpdate], IsNull(CONVERT(int,[OP_IsBook]), 0) AS OP_IsBook ";
            qry_dt = qry_dt + " , IsNull(CONVERT(int,[OP_DOCUMENT_QUERY]), 0) AS OP_DOCUMENT_QUERY ";
            qry_dt = qry_dt + " , IsNull(CONVERT(int,[OP_DOCUMENT_GET]), 0) AS OP_DOCUMENT_GET ";
            qry_dt = qry_dt + " FROM [dbo].[mk_options] ";
            qry_dt = qry_dt + " WHERE OP_DLKEY IN ";
            qry_dt = qry_dt + "   (select dl_key from tbl_DogovorList where dl_dgcod = @dgcode) ";
            qry_dt = qry_dt + " ORDER BY [OP_LastUpdate] DESC";
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter(qry_dt, Connection))
            {
                ...
                {
                    adapter.SelectCommand.Parameters.AddWithValue("@dgcode", dgCode);
                    int timeout = adapter.SelectCommand.CommandTimeout;
                    adapter.Fill(dt);
                }
                catch (System.Exception ex)
                {
                    TpLogger.Debug("GetBronirDocumentDates - DB QUERY", ex);
                }
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            DateTime row_dt = new DateTime(2000, 1, 1);
                            if (row.Field<DateTime>("OP_LastUpdate") != null) row_dt = row.Field<DateTime>("OP_LastUpdate");
                            if (row_dt.Year > 2000 && (row_dt < BronirDT || BronirDT.Year <= 2000)) BronirDT = row_dt;
                        }
                    }
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            DateTime row_dt = new DateTime(2000, 1, 1);
                            if (row.Field<DateTime>("OP_LastUpdate") != null) row_dt = row.Field<DateTime>("OP_LastUpdate");
                            if (row_dt.Year > 2000 && (row_dt < DocQueryDT || DocQueryDT.Year <= 2000)) DocQueryDT = row_dt;
                        }
                    }
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            DateTime row_dt = new DateTime(2000, 1, 1);
                            if (row.Field<DateTime>("OP_LastUpdate") != null) row_dt = row.Field<DateTime>("OP_LastUpdate");
                            if (row_dt.Year > 2000 && (row_dt < DocGetDT || DocGetDT.Year <= 2000)) DocGetDT = row_dt;
                        }
                    }
                }
            }
...
// NOT USED
//        public DataRow GetCruiseTarif(int dlKey)
//        {
//            DataTable dt = new DataTable();
//            string query = "SELECT actions_id, isNull(isRight, 0) as isRight, CDP_NAME, [Text] ";
//            query = query + " FROM [dbo].[mk_actions_options] ";
//            query = query + " LEFT JOIN dbo.MK_Cruise_DOPServise on actions_id = CDS_ID ";
//            query = query + " WHERE [actions_id] = -101 AND DL_key = @dlkey";
//            //query = query + " WHERE ([actions_id] = -101 OR [actions_id] = -112) AND DL_key = @dlkey"; 
//            using (SqlDataAdapter adapter = new SqlDataAdapter(query, Connection))
//            {
//                adapter.SelectCommand.Parameters.AddWithValue("@dlKey", dlKey);
//                adapter.Fill(dt);
...
//        }
        ...
        public DataTable GetDopServicesList()
        {
            DataTable dt = new DataTable();
            //string dop_qry = "SELECT [CDS_ID],[CDP_NAME] FROM [dbo].[MK_Cruise_DOPServise] ORDER BY CDS_ID DESC";
            ...
            using (SqlCommand com = new SqlCommand(dop_qry, WorkWithData.Connection))
            {
                SqlDataAdapter ad = new SqlDataAdapter(com);
                ad.Fill(dt);
            }
        ...
        public DataTable GetNoBonusesAndServices(int dlKey)
        {
            DataTable dt = new DataTable();
            //string query = @"SELECT actions_id, isNull(isRight, 0) as isRight, isNull(CDP_NAME, '') AS CDP_NAME, Text, ";
            //query = query + " isNull(CDP_BLOCK, 1) as BLOCK_ORDER, CASE WHEN actions_id = -101 then 1 else actions_id end OrdNum ";
            //query = query + " FROM [dbo].[mk_actions_options] left join dbo.MK_Cruise_DOPServise on actions_id = CDS_ID ";
            //query = query + " WHERE dl_key = @dlKey"; // actions_id < 0 AND 
            //query = query + " ORDER BY BLOCK_ORDER, CDP_ORDER, OrdNum";

            //string query = @"SELECT actions_id, isNull(isRight, 0) as isRight, ";
            //query = query + " CASE WHEN (LEN(CDP_NAME) = 0 OR CDP_NAME Is Null) AND actions_id > 0 THEN 'Бонусы' ELSE CDP_NAME END AS CDP_NAME, ";
            //query = query + " Text, ";
            //query = query + " isNull(CDP_BLOCK, 1) as BLOCK_ORDER, ";
            //query = query + " CASE WHEN actions_id = -101 THEN 4999 ";
            //query = query + "   ELSE case when actions_id = -102 then 9999 else isNull(CDP_ORDER + CDP_BLOCK * 1000, 0) end END AS NUM_ORDER ";
            //query = query + " FROM [dbo].[mk_actions_options] left join dbo.MK_Cruise_DOPServise on actions_id = CDS_ID ";
            //query = query + " WHERE dl_key = @dlKey AND actions_id < 0 "; // actions_id < 0 AND actions_id <> -101 
            //query = query + " ORDER BY NUM_ORDER";

            string query = @"SELECT actions_id, isNull(isRight, 0) as isRight, ";
            query = query + " CASE WHEN (LEN(CDP_NAME) = 0 OR CDP_NAME Is Null) AND actions_id > 0 THEN 'Бонусы' ELSE CDP_NAME END AS CDP_NAME, ";
            query = query + " Text, ";
            query = query + " isNull(CDP_BLOCK, 1) as BLOCK_ORDER, ";
            query = query + " case when actions_id = -102 then 9999 else isNull(CDP_ORDER + CDP_BLOCK * 1000, 0) end AS NUM_ORDER ";
            query = query + " FROM [dbo].[mk_actions_options] left join dbo.MK_Cruise_DOPServise on actions_id = CDS_ID ";
            query = query + " WHERE dl_key = @dlKey AND actions_id < 0 AND actions_id <> -101 AND actions_id <> -112";  
            query = query + " ORDER BY NUM_ORDER";
        ...
        public DataTable GetUnreadMessages(string dgCode)
        {
            ...
            using (
               SqlDataAdapter adapter =
                   new SqlDataAdapter(
                       "Select HI_ID,HI_MOD from History where HI_MOD in ('MTM','WWW') and HI_ID not in (select MS_HIID from dbo.mk_messageStatus where MS_isRead=1  and ms_uskey=@user) and HI_DGCOD =@dgCode order by HI_DATE desc",
                       Connection))
            {
                adapter.SelectCommand.Parameters.AddWithValue("@dgCode", dgCode);
                adapter.SelectCommand.Parameters.AddWithValue("@user", GetUserID());
        ...
        public void CruiseBonusesAndServicesAdd(int dlKey, int[] id)
        {
            ...
            {
                var add_query = @"insert into mk_actions_options (DL_key, actions_id) VALUES (@dlKey, @action_id)";
                using (SqlCommand com = new SqlCommand(add_query, Connection))
                {
                    com.Parameters.AddWithValue("@dlKey", dlKey);
                    com.Parameters.AddWithValue("@action_id", id[i]);
                    com.ExecuteNonQuery();
                }
            }
        }
...
// request TimeOut 
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter(procName, Connection))
            {
                ...
                {
                    TpLogger.Debug("GetRequests - Proc Command", ex);
                }
            }
            return dt;
        }
...
where isSuperViser = 1 or isProductManagers = 1 or isSalesManagers = 1 and isnull([is_realiz],0)=1 order by US_FullNameLat";
        ...
        public int GetUnansweredRequestsCount(string mod, int? usKey, bool comp_flag = false)
        {
            int val;
        ...
        public static List<string> GetRateList(string rate_keys)
        {
            List<string> list = new List<string>();
            if (string.IsNullOrEmpty(rate_keys)) return list;
            
            var query = @"SELECT * FROM [dbo].[Rates]";
            var dt = new DataTable();
            using (var adapter = new SqlDataAdapter(query, Connection))
            {
                adapter.Fill(dt);
            ...
            string[] keys = rate_keys.Split(',');
            for (int k = 0; k < keys.Length; k++)
            {
                if (!string.IsNullOrEmpty(keys[k]))
                {
                    int rate_key = int.Parse(keys[k]);
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        DataRow row = dt.Rows[i];
                        int key = row.Field<int>("ra_key");
                        string ra_code = row.Field<string>("RA_CODE");
                        if (key == rate_key)
                            list.Add(ra_code);
                    }
                }
