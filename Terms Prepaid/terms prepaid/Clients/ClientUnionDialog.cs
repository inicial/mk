...
{
    public partial class ClientUnionDialog : Form
    {
        SqlDataAdapter adapter = new SqlDataAdapter();

        public DataTable clientsDT = new DataTable("Clients");
        public DataTable card1 = new DataTable("card1");
        public DataTable card2 = new DataTable("card2");

        SqlConnection connection;
        long MANAGER_ID;
        int client1,  client2;
        public ClientUnionDialog()
        {
            InitializeComponent();
        }
        public ClientUnionDialog(int client1, int client2, long MANAGER_ID, SqlConnection connection)
        {
            InitializeComponent();
            this.MANAGER_ID = MANAGER_ID;
            this.connection = connection;
            ReUse(client1,client2);
        }
        public void ReUse(int client1, int client2)
        {
        ...
        public void UnionClients(bool StatRecalc)
        {
            DataRow dr;
            long clientDelete = -1, clientUpdate = -1;
            if (clientsDT.Rows.Count == 2)
            {
                if (radioButton1.Checked)
                {
                    dr = clientsDT.Rows[0];//Сохраняем в клиента1
                    clientUpdate = Convert.ToInt64(dr["CL_KEY"]);
                    clientDelete = Convert.ToInt64(clientsDT.Rows[1]["CL_KEY"]);
                }
                else
                {
                    dr = clientsDT.Rows[1];//Сохраняем в клиента2
                    clientUpdate = Convert.ToInt64(dr["CL_KEY"]);
                    clientDelete = Convert.ToInt64(clientsDT.Rows[0]["CL_KEY"]);
                }
                ReadFields(dr);
                ...
                try
                {
                    ...
                    adapter.SelectCommand =
                        new SqlCommand("select * from clients where CL_KEY=" + Convert.ToString(clientUpdate),
                                       connection);
                    ...
                    //Обновление туристов, оформленных на удаляемого клиента. Перепривязываются на нового.
                    DataTable tbl_Turist = new DataTable("tbl_Turist");
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select TU_KEY, TU_ID from tbl_Turist where TU_ID=" + Convert.ToString(clientDelete),
                            connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(tbl_Turist);
                    if (tbl_Turist.Rows.Count > 0) //Если есть подвешенные туристы
                    {
                        foreach (DataRow drt in tbl_Turist.Rows)
                        {
                            drt["TU_ID"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText = "UPDATE [tbl_Turist] SET [TU_ID] = @p2 WHERE [TU_KEY] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                        adapter.Update(tbl_Turist);
                    }


                    //Обновление дисконтных карт, оформленных на удаляемого клиента. Перепривязываются на нового.
                    DataTable Cards = new DataTable("Cards");
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select CD_Key, CD_CLKey from Cards where CD_CLKey=" + Convert.ToString(clientDelete),
                            connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Cards);
                    if (Cards.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Cards.Rows)
                        {
                            drt["CD_CLKey"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText = "UPDATE [Cards] SET [CD_CLKey] = @p2 WHERE [CD_Key] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                        adapter.Update(Cards);
                    }
                    //Обновление привязок к пассажирам
                    DataTable Lanta_ClientPassangerLink = new DataTable("Lanta_ClientPassangerLink");
                    adapter.SelectCommand =
                        new SqlCommand(
                            @"
                SELECT    CPL_ID, CPL_ClientКеу
                FROM         Lanta_ClientPassangerLink
                WHERE     CPL_ClientКеу = " +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientPassangerLink);
                    if (Lanta_ClientPassangerLink.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientPassangerLink.Rows)
                        {
                            drt["CPL_ClientКеу"] = clientUpdate; //Удалить дубли!!!
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText =
                            "UPDATE [Lanta_ClientPassangerLink] SET [CPL_ClientКеу] = @p2 WHERE [CPL_ID] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                    ...
                    /*
                     * DataTable dubles = new DataTable("dubles");
                    SELECT     CPL_ClientКеу, CPL_PassangerКеу
                    FROM         Lanta_ClientPassangerLink
                    GROUP BY CPL_ClientКеу, CPL_PassangerКеу
                    HAVING      (COUNT(CPL_PassangerКеу) > 1)*/
                    ...
                    adapter.SelectCommand =
                        new SqlCommand(
                            @"
                SELECT CF_ID, CF_CLKey
                FROM Lanta_ClientFamily WHERE CF_CLKey = " +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientFamily);
                    if (Lanta_ClientFamily.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientFamily.Rows)
                        {
                            drt["CF_CLKey"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        //adapter.UpdateCommand.CommandText = "UPDATE [Lanta_ClientFamily] SET [CF_CLKey] = @p1 WHERE [CF_ID] = @p2";
                        adapter.UpdateCommand.Transaction = transaction;
                        adapter.Update(Lanta_ClientFamily);
                    }
                    //Если объединённый клиент оказался сразу в нескольких семьях - эти семьи считаются одной семьёй и их объединяем.
                    Lanta_ClientFamily.Clear();
                    adapter.SelectCommand =
                        new SqlCommand(
                            @"
                    SELECT CF_Community, CF_CLKey
                    FROM Lanta_ClientFamily WHERE CF_CLKey = " +
                            Convert.ToString(clientUpdate), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientFamily);
                    if (Lanta_ClientFamily.Rows.Count > 1)
                    {
                        //Семья, в которую объединятся все семьи
                        int FamilyIDUpdate = Convert.ToInt32(Lanta_ClientFamily.Rows[0]["CF_Community"]);
                        int FamilyIDDelete = -1;
                        foreach (DataRow drt in Lanta_ClientFamily.Rows)
                        {
                            FamilyIDDelete = Convert.ToInt32(drt["CF_Community"]);
                            SqlCommand cmd =
                                new SqlCommand(
                                    "update Lanta_ClientFamily set CF_Community=" + FamilyIDUpdate.ToString() +
                                    " where CF_Community=" + FamilyIDDelete.ToString(), connection, transaction);
                            cmd.ExecuteNonQuery();
                        }
                    }
                    //Обновление истории предыдущих привязок, оформленных на удаляемого клиента. Перепривязываются на нового.
                    DataTable Lanta_ClientDeleteHistory = new DataTable("Lanta_ClientDeleteHistory");
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select CDH_ID, CDH_CLKeyNew from Lanta_ClientDeleteHistory where CDH_CLKeyNew=" +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientDeleteHistory);
                    if (Lanta_ClientDeleteHistory.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientDeleteHistory.Rows)
                        {
                            drt["CDH_CLKeyNew"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText =
                            "UPDATE [Lanta_ClientDeleteHistory] SET [CDH_CLKeyNew] = @p2 WHERE [CDH_ID] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                        adapter.Update(Lanta_ClientDeleteHistory);
                    }

                    //Обновление статистики удаляемого клиента. Перепривязываются на нового.
                    DataTable Lanta_ClientStatDogovor = new DataTable("Lanta_ClientStatDogovor");
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select CSD_ID, CSD_CLКеу from Lanta_ClientStatDogovor where CSD_CLКеу=" +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientStatDogovor);
                    if (Lanta_ClientStatDogovor.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientStatDogovor.Rows)
                        {
                            drt["CSD_CLКеу"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText =
                            "UPDATE [Lanta_ClientStatDogovor] SET [CSD_CLКеу] = @p1 WHERE [CSD_ID] = @p2";
                        adapter.UpdateCommand.Transaction = transaction;
                    ...
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select CSL_ID, CSL_CLKEY from  Lanta_ClientStatDogovorList where CSL_CLKEY=" +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientStatDogovorList);
                    if (Lanta_ClientStatDogovorList.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientStatDogovorList.Rows)
                        {
                            drt["CSL_CLKEY"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText =
                            "UPDATE [Lanta_ClientStatDogovorList] SET [CSL_CLKEY] = @p2 WHERE [CSL_ID] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                    ...
                    adapter.SelectCommand =
                        new SqlCommand(
                            "select CSS_ID, CSS_CLКеу from Lanta_ClientStatSputniki where CSS_CLКеу=" +
                            Convert.ToString(clientDelete), connection);
                    adapter.SelectCommand.Transaction = transaction;
                    adapter.Fill(Lanta_ClientStatSputniki);
                    if (Lanta_ClientStatSputniki.Rows.Count > 0)
                    {
                        foreach (DataRow drt in Lanta_ClientStatSputniki.Rows)
                        {
                            drt["CSS_CLКеу"] = clientUpdate;
                        }
                        builder = new SqlCommandBuilder(adapter);
                        adapter.UpdateCommand = builder.GetUpdateCommand();
                        adapter.UpdateCommand.CommandText =
                            "UPDATE [Lanta_ClientStatSputniki] SET [CSS_CLКеу] = @p2 WHERE [CSS_ID] = @p1";
                        adapter.UpdateCommand.Transaction = transaction;
                        adapter.Update(Lanta_ClientStatSputniki);
                    }
                    transaction.Commit();
                    //Удаление ненужного клиента
                    ClientsMainForm cmf = new ClientsMainForm("", MANAGER_ID, connection, false);
                    cmf.DeleteClient(clientDelete, clientUpdate);

                    if (StatRecalc)
                    {
                        //Подсчёт статистики
                        EditClient ec = new EditClient(clientUpdate, MANAGER_ID, connection, false, false);
                        ec.UpdateStatistic();
                        ec.SaveClient(false);
                    }
                }
                ...
                {
                    MessageBox.Show("Слияние не удачно - " + ex.Message, "Ошибка", MessageBoxButtons.OK,
                                    MessageBoxIcon.Error);
                    transaction.Rollback();
                }
                //string[] ConnectionStringNative = connection.ConnectionString.Split(';');
                //SqlConnection connSys = new SqlConnection(String.Format("{2};{3};User ID={0}; pwd={1}; Timeout=30", "sysadm", "ieheg", ConnectionStringNative[0], ConnectionStringNative[1])); ;
                //if (connSys.State != ConnectionState.Open)
                //    connSys.Open();
                //SqlTransaction transaction2 = connSys.BeginTransaction("Transaction");
                //try
                //{
                //    string[] dataBases = new string[] { "lanta03", "lanta04", "lanta05", "lanta06", "lanta07", "lanta08" };

                //    foreach (string dbName in dataBases)
                //    {
                //        connSys.ChangeDatabase(dbName);
                //        //Обновление туристов, оформленных на удаляемого клиента. Перепривязываются на нового.
                //        tbl_Turist = new DataTable("tbl_Turist");
                //        adapter.SelectCommand = new SqlCommand("select TU_KEY, TU_ID from tbl_Turist where TU_ID=" + Convert.ToString(clientDelete), connSys);
                //        adapter.SelectCommand.Transaction = transaction2;
                //        adapter.Fill(tbl_Turist);
                //        if (tbl_Turist.Rows.Count > 0)//Если есть подвешенные туристы
                //        {
                //            foreach (DataRow drt in tbl_Turist.Rows)
                ...
                //            adapter.UpdateCommand = builder.GetUpdateCommand();
                //            adapter.UpdateCommand.CommandText = "UPDATE [tbl_Turist] SET [TU_ID] = @p2 WHERE [TU_KEY] = @p1";
                //            adapter.UpdateCommand.Transaction = transaction2;
                //            adapter.Update(tbl_Turist);
                //        }
                //    }
                //}
        ...
        {
            clientsDT.Clear();
            adapter.SelectCommand = new SqlCommand(@"select * FROM Clients where CL_KEY=" + Convert.ToString(client1),connection);
            adapter.Fill(clientsDT);
            adapter.SelectCommand.CommandText =@"select * FROM Clients where CL_KEY=" + Convert.ToString(client2);
            adapter.Fill(clientsDT);           
            
            if (clientsDT.Rows.Count == 2)
            {
                adapter.SelectCommand = new SqlCommand(@"SELECT     CARDS.CD_Key, CARDS.CD_Date
                FROM         Clients LEFT OUTER JOIN
                      CARDS ON Clients.CL_KEY = CARDS.CD_CLKey
                WHERE     Clients.CL_KEY =" + Convert.ToString(clientsDT.Rows[0]["CL_KEY"]), connection);
                adapter.Fill(card1);
                adapter.SelectCommand = new SqlCommand(@"SELECT     CARDS.CD_Key, CARDS.CD_Date
                FROM         Clients LEFT OUTER JOIN
                      CARDS ON Clients.CL_KEY = CARDS.CD_CLKey
                WHERE     Clients.CL_KEY =" + Convert.ToString(clientsDT.Rows[1]["CL_KEY"]), connection);
                adapter.Fill(card2);
        ...
        private void PopulateFields()
        {
            ...
            if (clientsDT.Rows.Count == 2)
            {
                if (card1.Rows.Count > 0 && card1.Rows[0]["CD_Date"] != System.DBNull.Value)
                    textBox1_CD_Date.Text = Convert.ToDateTime(card1.Rows[0]["CD_Date"]).ToString("d");
                
                 if (card2.Rows.Count > 0 && card2.Rows[0]["CD_Date"] != System.DBNull.Value)
                    textBox2_CD_Date.Text = Convert.ToDateTime(card2.Rows[0]["CD_Date"]).ToString("d"); 
              
                dr1 = clientsDT.Rows[0];
                dr2 = clientsDT.Rows[1];
                foreach (DataColumn dc in clientsDT.Columns)
                {
        ...
        private void MergeProcedure()
        {
            ...
            if (radioButton1.Checked)
            {
                textBox3_CL_KEY.Text = textBox1_CL_KEY.Text;
