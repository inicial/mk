...
namespace terms_prepaid.UserControls
{
    public partial class ucAnnulation : UserControl
    {
        ...
        private void btnPetitionView_Click(object sender, EventArgs e)
        {
            ...
            var row = WorkWithData.GetPetition2(num);

            var data = row.Field<byte[]>("data");
            var ext = row.Field<string>("ext");

            string fileName = _tempDirectory + Path.PathSeparator + "Petition" + num.ToString() + "." + ext;
