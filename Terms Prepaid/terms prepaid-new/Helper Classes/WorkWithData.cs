...
where OP_DLKEY = @dlKey";
        ...
        private const string getVoucherInfo = @"select DG_DISCOUNT, DG_DISCOUNTSUM, DG_TYPECOUNT, DG_PAYMENTDATE, DG_PPAYMENTDATE, DG_PRICE, DG_PROCENT, DG_RazmerP, DG_PRICE, DG_RATE, DG_PARTNERKEY  
from tbl_Dogovor
...
(SELECT --row_number() over (order by mp_problemCode) as rownum
  ...
  FROM [dbo].[AnnulReasons]", Connection))
        ...
        public static ChangeData GetServiceSettingLastChange(int dlKey, string type)
        {
            DataTable dt;

            using (SqlDataAdapter adapter = new SqlDataAdapter(selectServiceSettingLastChange, _connection))
            {
                dt = new DataTable();
                adapter.SelectCommand.Parameters.AddWithValue("@dlKey", dlKey);
                adapter.SelectCommand.Parameters.AddWithValue("@alias", type);
                adapter.Fill(dt);
                ...
                if (dt.Rows != null)
                {
                    ...
                    {
                        string alias = row.Field<string>("HD_Alias");
                        if (alias == "DL_PAYMENTDATE")
                            new ChangeData() { Date = row.Field<DateTime>("date"), Who = row.Field<string>("who") };
                    }
                }
            }
            ...
            {
                return new ChangeData() { Date = dt.Rows[0].Field<DateTime>("date"), Who = dt.Rows[0].Field<string>("who") };
            }

            return null;
        }

        public static ChangeData GetServiceSettingLastChangeOld(int dlKey, string type)
        {
            DataTable dt;
        ...
        public static string GetSuperProblem()
        {
            string rezult = string.Empty;
            int i = 1;
            using (SqlDataAdapter adapter = new SqlDataAdapter(WorkWithData.SearchDogovorProc, Connection))
            {
                adapter.SelectCommand.CommandType=CommandType.StoredProcedure;
  ...
  order by CDP_ORDER  ", Connection);
        ...
        */
        ...
        protected void AddParam(SqlCommand com, string name, Object value)
        {
            com.Parameters.AddWithValue(name, value ?? DBNull.Value);
        }

        protected void AddParam(SqlDataAdapter adapter, string name, Object value)
        {
...
left join dbo.UserList as ul on ul.US_KEY = rq.US_KEY";
        ...
        private DataTable ArrayToDataTable(IEnumerable<int> data)
        {
            var dt = new DataTable();
            dt.Columns.Add();
        ...
        public DataTable GetRequests(DateTime? dateBegin, DateTime? dateEnd, string senderAddress, int? usKey, int[] problems, int[] statuses, bool senderAddressContains)
        {
            var problemsTable = ArrayToDataTable(problems);
            var statusesTable = ArrayToDataTable(statuses);

            var procName = senderAddressContains ? "mk_GetRequestsNew3_like" : "mk_GetRequestsNew3";
        ...
        {
            var query = @"select * from mk_RequestSubStatuses where IsProblem = 1";
...
where isSuperViser = 1 or isProductManagers = 1 or isSalesManagers = 1 and isnull([is_realiz],0)=1 order by US_FullNameLat";
        ...
        public int GetUnansweredRequestsCount(string mod)
        {
            var query = @"select Count(ms.RequestId) from
                    (SELECT *, ROW_NUMBER() OVER (PARTITION BY RequestId ORDER BY [Date] desc) AS RowNumber FROM mk_RequestMessages) ms
                    inner join mk_Requests as rq on rq.Id = ms.RequestId
                    where RowNumber = 1 and IsIncoming = 1 and mod = @mod and rq.RequestStatusId < 3 and isNull(rq.IsClosed, 0) != 1";
        ...
        public int GetUnansweredRequestsCount(string mod, int usKey)
        {
            var query = @"select Count(ms.RequestId) from
                (SELECT *, ROW_NUMBER() OVER (PARTITION BY RequestId ORDER BY [Date] desc) AS RowNumber FROM mk_RequestMessages) ms
                inner join mk_Requests as rq on rq.Id = ms.RequestId
                where RowNumber = 1 and IsIncoming = 1 and mod = @mod and rq.RequestStatusId < 3 and rq.US_KEY = @usKey and isNull(rq.IsClosed, 0) != 1";
