...
namespace terms_prepaid.Forms
{
    public partial class frmAccounts : Form
    {
        ...
        //private const string selectAccounts = "Select Ac_Name,AC_URL from mk_AccountsURL where AC_TYPE in({0},-1) and AC_VISIBLE = 1{1}";
...
//            values (@dgCode,GetDate(),@Who,@Text,@mod)";
        ...
        void GetDate()
        {
            /*var prepaymenCondition = " and AC_URL not like('%view=prepayment%')";
            accountTable= new DataTable();
            using (SqlDataAdapter adapter = new SqlDataAdapter(string.Format(selectAccounts, _type, prepaymenCondition),WorkWithData.Connection))
            {
                adapter.Fill(accountTable);
            
            }*/
            
            lbAccounts.DataSource = WorkWithData.GetAccounts(_dgcode, _type);
