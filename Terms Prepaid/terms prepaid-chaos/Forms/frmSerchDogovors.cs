    ...
    public partial class frmSerchDogovors : Form
    {
       ...
       // private DataTable _option3, _noCreateInshur, _noAcceptUsluga, _noDogovorAccept, _noInsertDocument, _general;
        ...
        void lbTempM_Click(object sender, EventArgs e)
        {
            ...
            _dogovors = WorkWithData.GetUnhandledCorrespondence2(null);
            tbMessages.Text = _dogovors.Rows.Count.ToString();
        ...
        void lbTempM1_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            _dogovors = WorkWithData.GetUnhandledCorrespondence2(WorkWithData.GetUserID());
            ((tableLayoutPanel3.Controls["tlpMessage"] as TableLayoutPanel).Controls["tbMyM"] as TextBox).Text = _dogovors.Rows.Count.ToString();
...
При не соблюдении сроков оформления заказа и оплаты, круизная компания может аннулировать Вашу бронь. ";
        ...
        private void timeRefreshMessage_Tick(object sender, EventArgs e)
        {
            ...
            var myMessages = WorkWithData.GetUnhandledCorrespondence2(WorkWithData.GetUserID());
            ...
            if (_access.isSuperViser)
            {
                ...
                var allMessages = WorkWithData.GetUnhandledCorrespondence2(null);
                tbMessages.Text = allMessages.Rows.Count.ToString(); //count.ToString();
                ((tableLayoutPanel3.Controls["tlpMessage"] as TableLayoutPanel).Controls["tbMyM"] as TextBox).Text =
                    myMessages.Rows.Count.ToString();
            }
            else
            {
                tbMessages.Text = myMessages.Rows.Count.ToString(); //count.ToString();
            }
        }
        ...
        {
            frmUnhandledCorrespondence.GetUnhandledCorrespondence(ShowUnhandledCorrespondence);
        }

        private void ShowUnhandledCorrespondence(DataTable dt)
        {
            _dogovors = dt;

            if(_dogovors.Columns.IndexOf("Button") == -1) _dogovors.Columns.Add("Button", typeof(Image));
            if (_dogovors.Columns.IndexOf("Button1") == -1) _dogovors.Columns.Add("Button1", typeof(Image));

            foreach (DataRow row in _dogovors.Rows)
            {
                row["Button"] = Properties.Resources.mailIconMin;
                row["Button1"] = Properties.Resources.delete;
            }
        ...
        {
            btnNewMessages.BackColor = btnNewMessages.BackColor == Color.Green ? SystemColors.Control : Color.Green;
        }
