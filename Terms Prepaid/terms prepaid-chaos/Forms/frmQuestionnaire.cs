...
namespace terms_prepaid
{
    public partial class frmQuestionnaire : Form
    {
        ...
        const string select_country = @"select [CN_KEY],[CN_NAME] , case when isnull(cn_code,'')<>'' then 1 else 0 end  as visible   from tbl_Country  where  isnull(cn_code,'')<>'no' order by case when isnull(cn_code,'')<>'' then cn_code else 'ЯЯЯЯЯЯ' end , CN_NAME";
        ...
        ,[TU_EMAIL]=@TUEMAIL
        ,[TU_SEX]=@TUSEX
        ,[TU_RealSex]=@TUREALSEX
        where TU_KEY=@tukey";
        private List<string> _Countrys;
        private List<string> _BirthCountrys;
        ...
        public frmQuestionnaire(int tu_key,int flag)
        {
        ...
        void AutoCompleteTest(TextBox tb)
        {
        ...
        private void FillForm(DataRow row)
        {
            ...
            birthCity = row.Field<string>("TU_BIRTHCITY");
            cbBirthCountry.Text = row.Field<string>("TU_BIRTHCOUNTRY");
            ...
            tbBild.Text = row.Field<string>("TU_HOUSE");
            ...
            cbSex.SelectedIndex = GetSexIndex(row.Field<Int16?>("TU_SEX"), row.Field<Int16?>("TU_RealSex"));
        }

        //private

        public static int GetSexIndex(int? nSex, int? nRealSex)
        {
            int sex = nSex ?? 0;
            
            if (nRealSex == null && (nSex != 0 || nSex != 1))
                return -1;

            int realSex = nRealSex ?? sex;

            return sex < 2 ? realSex : sex == 2 ? 2 + realSex : sex == 3 ? 4 + realSex : -1;
        }
        ...
        public static int? GetSex(int index)
        {
            ...
            return index < 2 ? index : index < 4 ? 2 : 3;
        }

        public static int? GetRealSex(int index)
        {
            if (index < 0 || index > 5)
                return null;
        ...
        void GetDate()
        {
            ...
            cbBirthCountry.DataSource = new List<string>(_Countrys);
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            using (SqlCommand com = new SqlCommand(updateTurist,WorkWithData.Connection))
            {
                ...
                com.Parameters.AddWithValue("@TUBIRTHCOUNTRY", cbBirthCountry.Text);

                if (birthCity != null)
                    com.Parameters.AddWithValue("@TUBIRTHCITY", birthCity);
                else
                    com.Parameters.AddWithValue("@TUBIRTHCITY", DBNull.Value);
                ...
                com.Parameters.AddWithValue("@TUSEX", GetSex(cbSex.SelectedIndex));
                com.Parameters.AddWithValue("@TUREALSEX", GetRealSex(cbSex.SelectedIndex));
