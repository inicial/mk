...
namespace WpfControlLibrary.Model.RequestJournal
{
    ...
    public class RequestJournalSender : IRequestJournalSender
    {
        ...
        private string _nullPixelUrl = "http://mcruises.ru/workplace/getimage.php?eid";
        ...
        private DateTime SendHtmlMessage(SmtpClient client, RequestMessage msg, bool deliveryNotification)
        {
            ...
            var senderAddress = !string.IsNullOrEmpty(Config.SenderAddress) ? Config.SenderAddress : msg.SenderAddress;

            var message = new MailMessage(senderAddress, msg.DestinationAddress, msg.Theme, htmlWithGoogleAnalytics)
            {
                IsBodyHtml = true
            };

            new InlineImgHelper().AttachImages(message);
