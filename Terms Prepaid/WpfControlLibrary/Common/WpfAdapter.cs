...
host.Child = null; // prevent assigning the same object
        ...
        public static BillSettingViewModel AttachBillSetting(ElementHost host)
        {
            var view = new BillSettingView();
            var model = new BillSettingViewModel();
            view.DataContext = model;
            view.InitializeComponent();
            host.Child = view;
            return model;
        ...
        public static void ShowRequestsJournal(int mode)
        {
            ...
            var model = new RequestsJournalHostViewModel();
            view.DataContext = model;
            RequestsJournalViewModel vm = (RequestsJournalViewModel)model.Vm;
            if (vm != null) vm.JournalWindow = view.JournalWindow;
            ElementHost.EnableModelessKeyboardInterop(view);

            view.Title = "Журнал заявок";
            vm.WindowTitle = "ЖУРНАЛ  ЗАЯВОК";
            model.Vm.ShowDataGrid();
            view.Show();

            if (mode == 1)
            {
                model.Vm.ShowAllRequests();
            }
            else if (mode == 2)
            {
                model.Vm.ShowAllMyRequests();
            }
            else if (mode == 3)
            {
                view.Title = "Журнал попутчиков";
                vm.WindowTitle = "ЖУРНАЛ  ПОПУТЧИКОВ";
                vm.SetCompanionMode(true);
                model.Vm.ShowAllCompanions();
            }
            else if (mode == 4)
            {
                view.Title = "Журнал попутчиков";
